...
export type ExecuteOnePendingJobResult = {
  httpStatus: number;
...
/** Runs at most one pending job (shared by /api/jobs/run and cron drain). */
export async function executeOnePendingJob(projectId?: string): Promise<ExecuteOnePendingJobResult> {
  ...
  if (!job) {
    ...
    return { httpStatus: 200, body: { message: "all_done" } };
  }
  ...
  try {
    ...
    switch (job.type as JobType | string) {
        ...
        for (let i = 0; i < cards.length; i += concurrency) {
          ...
          const results = await Promise.all(
            batch.map(async (card) => {
              const { classifyCard, classifyWithHeuristics, estimateClassificationConfidence } = await import(
                "@/lib/llm/classify"
              );
              const llmOpts = {
              ...
              try {
                result = await classifyCard(classifyInput, llmOpts);
              } catch (err) {
                source = "heuristic";
                result = classifyWithHeuristics(classifyInput);
                rawOutput = `heuristic_fallback: ${err instanceof Error ? err.message : String(err)}`;
                logger.warn("Classification fallback used", {
                  cardId: card.id,
                  reason: rawOutput,
                });
              }
              await prisma.lLMAssessment.upsert({
                ...
                create: {
                  ...
                  ...result,
                  source,
                  confidence: estimateClassificationConfidence(source, result),
              ...
              return { cardId: card.id, usedFallback: !!rawOutput };
            })
          );
          done += batch.length;
          payload = await updateJobPayload(job.id, payload, {
          ...
          if (fallbackCount > 0) {
            logger.warn("Classification completed with fallbacks", {
              fallbackCount,
      ...
      case "cluster": {
        ...
        const roundId = asString(payload.roundId);
        if (!projectId || !roundId) {
          throw new Error("Missing projectId or roundId for cluster");
        }
        ...
        try {
          ...
          if (roundData) {
            const { generateExecutiveSummary, buildHeuristicExecutiveSummary } = await import(
              "@/lib/llm/summary"
            );
            const { classifyWithHeuristics } = await import("@/lib/llm/classify");
            const classified = roundData.cards.map((c) => ({
              comment: c.comment.message,
              frameName: c.frameName,
              classification: (c.assessment ??
                classifyWithHeuristics({
                  commentText: c.comment.message,
                  authorName: "unknown",
                  frameName: c.frameName,
                  pageName: c.pageName,
                })) as unknown as import("@/lib/llm/schemas").Classification,
            }));
            ...
            let summarySource: "llm" | "heuristic" = config2?.skipLlm ? "heuristic" : "llm";
            let summary = buildHeuristicExecutiveSummary(summaryInput);
            if (!config2?.skipLlm && classified.length > 0) {
              try {
                summary = await generateExecutiveSummary(summaryInput, {
                  provider: config2?.llmProvider,
                  model: config2?.llmModel,
                  apiKey: config2?.llmApiKey,
                });
              } catch (summaryErr) {
                summarySource = "heuristic";
                logger.error("Executive summary failed, using heuristic fallback", {
                  roundId,
                  error:
        ...
        try {
          ...
          if (finalRound && integrationConfig) {
            const rankedClusters = rankIssueClusters(finalRound.clusters);
            const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
            const digestUrl = `${baseUrl}/project/${projectId}/digest?roundId=${roundId}`;
            const criticalCount = rankedClusters.filter((c) =>
              c.cards.some((card) => card.assessment?.priorityHint === "critical")
            ).length;
            const topIssues = rankedClusters.slice(0, 5).map((c) => ({
              title: c.title,
              priority:
            ...
            if (integrationConfig.notifySyncComplete) {
              ...
              await sendPushToAll({
                ...
                body: `${rankedClusters.length} issue${rankedClusters.length !== 1 ? "s" : ""} found${criticalCount > 0 ? ` (${criticalCount} critical)` : ""}`,
                url: digestUrl,
              });
            }
          }
